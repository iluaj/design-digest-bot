...
Самый удобный способ: бот живёт в GitHub Actions, ваш компьютер не нужен,
платить не за что.

1. Сделайте форк этого репозитория (можно приватный).
2. Settings → Secrets and variables → Actions → New repository secret:
   - `BOT_TOKEN` — токен от @BotFather
   - `CHAT_ID` — ваш ID (узнаётся командой `python run.py whoami`)
3. Вкладка Actions → «Дайджест» → Run workflow. Первый запуск освоит все
   источники молча, дальше будет приходить только новое.

Дальше он работает сам: каждый день в 18:00 по Москве. Время меняется в
`.github/workflows/digest.yml`, строка `cron` (там время по UTC, Москва — плюс
три часа).

**Как хранится состояние.** Между запусками облако ничего не помнит, поэтому
отпечатки уже отправленных работ лежат в репозитории — `data/seen.txt`, около
320 КБ. Базу SQLite туда класть нельзя: она весит мегабайты двоичных данных, и
за год репозиторий распух бы до гигабайтов. Текстовый файл git дописывает
построчно — обычный день добавляет десяток строк.

**Чего не будет.** Команды в чате (`/now`, `/stats` и прочие) работать не
будут: отвечать на них некому, в облаке ничего не крутится между запусками.
Расписание GitHub Actions может плавать на 5–15 минут. Если нужны команды и
точное время — поднимайте демон на своём сервере (см. systemd выше).

**Часть источников в облаке недоступна.** Защита от ботов у ряда сайтов режет
серверные адреса дата-центров: с домашнего интернета они открываются, из
GitHub Actions отдают 403 или 429. Бот пытается обойти это, повторяя запрос с
TLS-отпечатком настоящего Chrome (`curl_cffi`) — так удаётся вернуть около
двух третей заблокированных. Оставшиеся 8–9 источников из 335 в облаке
недоступны и просто пропускаются до следующего дня.
...
**Часть сайтов недоступна с серверных адресов** — см. раздел про запуск в
облаке. На своей машине они работают.
...
data/seen.txt     отпечатки отправленных работ (состояние для облака)
sources_extra.yaml  выведенные из дайджеста источники, бот их не читает
```
